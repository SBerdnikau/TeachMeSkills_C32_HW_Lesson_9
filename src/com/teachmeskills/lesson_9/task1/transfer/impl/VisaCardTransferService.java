...
public class VisaCardTransferService implements CardTransferService {
    ...
    public Check transferFromCardToCard(BaseCard cardFrom, BaseCard cardTo, int amountTransfer) {
        if (cardFrom.checkCardLimitTransfer(amountTransfer)) {
            ...
            return new Check(amountTransfer, new Date(), cardFrom.getCardNumber(), cardTo.getCardNumber(), "Transfer limit exceeded. Transfer limit is = " +  Constans.TRANSFER_LIMIT_VISA_CARD );
        }
    }
    ...
    public Check transferFromCardToAccount(BaseCard cardFrom, Account accountTo, int amountTransfer) {
        if (cardFrom.checkCardLimitTransfer(amountTransfer)) {
            ...
            return new Check(amountTransfer, new Date(), cardFrom.getCardNumber(), accountTo.getAccountNumber(), "Transfer limit exceeded. Transfer limit is = " +  Constans.TRANSFER_LIMIT_VISA_CARD );
        }
    }
}
